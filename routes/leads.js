...
//       const { search, status, priority, source, assignedTo, service } = req.query;
...
//       // Allow explicit assignedTo filter for admin only
...
//       // LIMIT/OFFSET interpolated as integers to avoid ER_WRONG_ARGUMENTS
//       const leadsSql = `
//         SELECT 
//           l.*,
...
//       const [leads] = await pool.execute(leadsSql, queryParams);
...
//     sanitizeParams(leadId, req.user.userId)
//   );
...
//           assigned_to, estimated_value, notes, expected_close_date, whatsapp_number,
//           service
//         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
//       `,
//         sanitizeParams(
...
//             id, name, email, phone, company, status, source, assigned_to,
//             tags, notes, total_value, whatsapp_number
//           ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
//         `,
//           sanitizeParams(
//             customerId,
...
//testing for changes (16-12-2025)
...
//       if (status) {
//         // If UI explicitly asks for a status, respect it
//         whereClause += " AND l.status = ?";
//         queryParams.push(status);
//       } else {
//         // Default: hide closed-won from the main lead list
//         whereClause += " AND l.status != 'closed-won'";
//       }
  ...
  async (req, res) => {
    try {
      ...
      try {
        ...
        // Safe tags parsing – avoids SyntaxError: Unexpected end of JSON input
        try {
