...
// Build renewal from customer recurring settings + request body
// Assumes customers table has: recurring_enabled, recurring_interval, recurring_amount, recurring_service, next_renewal_date, default_renewal_status, default_renewal_reminder_days, default_renewal_notes.
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const [renewals] = await pool.execute(renewalsSql, queryParams);
...
router.put(
  ...
  [
    ...
    body("service").optional().trim().notEmpty().withMessage("Service cannot be empty"),
    body("amount").optional().isNumeric().withMessage("Amount must be numeric"),
...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    await pool.execute("DELETE FROM renewals WHERE id = ?", [id]);
