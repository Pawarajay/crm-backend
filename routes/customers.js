...
// // Helper: handle validation errors consistently
...
// // RULE: admin sees all; non-admin sees only their own (assigned_to = req.user.userId)
// router.get(
//   "/",
//   authenticateToken,
//   [
...
//       // Build SQL with LIMIT/OFFSET interpolated as integers
//       const customersSql = `
//         SELECT 
//           c.*,
...
//       // Use execute with only filter params
//       const [customers] = await pool.execute(customersSql, queryParams);
...
// // CREATE new customer
// // RULE: assign_to = current user for both admin and normal users
// router.post(
//   "/",
//   authenticateToken,
//   [
...
//           id, name, email, phone, company, address, city, state, zip_code, country,
//           status, source, tags, notes, total_value, whatsapp_number, assigned_to
//         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
//       `,
//         sanitizeParams(
//           id,
...
// // UPDATE customer
// // RULE: non-admin can only update their own customers
// router.put(
//   "/:id",
//   authenticateToken,
//   [
...
// // DELETE customer
// // RULE: non-admin can only delete their own customers
...
//     // service / invoice defaults / recurring / renewals
...
//           source,                    // 12
//           assignedTo,                // 13
//           JSON.stringify(tags || []),// 14
//           notes,                     // 15
//           null,                      // 16 last_contact_date
//           totalValueNumber,          // 17 total_value
//           oneTimePrice != null && oneTimePrice !== "" ? Number(oneTimePrice) : null, // 18
//           monthlyPrice != null && monthlyPrice !== "" ? Number(monthlyPrice) : null, // 19
//           manualPrice != null && manualPrice !== "" ? Number(manualPrice) : null,    // 20
//           defaultTaxRate,            // 21
//           defaultDueDays,            // 22
//           defaultInvoiceNotes,       // 23
//           whatsappNumber,            // 24
//           service,                   // 25
//           serviceType || null,       // 26
//           recurringEnabled ? 1 : 0,  // 27
//           recurringInterval || "monthly", // 28
//           recurringAmount,           // 29
//           recurringService,          // 30
//           nextRenewalDate,           // 31
//           defaultRenewalStatus,      // 32
//           defaultRenewalReminderDays,// 33
//           defaultRenewalNotes        // 34
//         )
//       );
...
//       // normalise numeric pricing fields
...
router.post(
  "/:id/move-to-lead",
  authenticateToken,
  // ❌ remove the validators array completely
  async (req, res) => {
    try {
      // no handleValidation call now
      const { id } = req.params;
      ...
      // const [existingLeadByEmail] = await pool.execute(
      ...
      //     leadId: existingLeadByEmail[0].id,
      //   });
      // }
